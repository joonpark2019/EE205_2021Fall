...
void subtract(char *a, char *b)
{
    ...
    for (int i = 0; i < n; i++)
    {
        if ( a[i] >= ( getDigit(b, i, m) + carry ))
            { a[i] = a[i] - getDigit(b,i, m) - carry; carry = '0'; }
        else { a[i] = a[i] + (char)10 - getDigit(b,i,m) - (carry - '0'); carry = '1';}
    }
    //assert(carry == '0');
}
...
int main(const int argc, const char *argv[])
{
    ...
    while( fgets(buff1, MAXLENGTH + 1, stdin) && fgets(buff2, MAXLENGTH + 1, stdin))
    {
        ...
        int n = (int)strlen(buff1);
        buff1[n-1] = '\0';
        n = (int)strlen(buff1);
        
        int m = (int)strlen(buff2);
        ...
        //take each character in the string and then cast as an integer???
        //the final product, turn into a string and cast each character as a character???
        ...
        printf("Time taken by gradeschool: %ld micro seconds\n",
