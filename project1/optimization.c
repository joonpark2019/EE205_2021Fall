...
void mult(const char *a, const char b, char *p, int n, int j) //n is the length of a
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        //atimesb[l - i - 1] 
        p[p_len - 1 - j - i] = digitAdd(d, *cprev, *carry, carry); 
        *cprev = *c;
    }
    d = '0';
    //possibly error here due to index of 0
    p[p_len - 1 - j - n - 1] = digitAdd(d, *cprev, *carry, carry);
...
char* Karatsuba(char *a, char *b)
{
  ...
  //  printf("a0 %s\n", a0); printf("a1 %s\n", a1); printf("b0 %s\n", b0);
   ...
   // printf("p0 = a0 * b0 is %s\n", p0);
    ...
    //printf("p is %s\n", p);
    subtract(p1, p0); subtract(p1, p2);//printf("diff %s\n",p1);  
    //printf("diff %s\n",p1);
    addAt(p, p1, k, n+m);
...
/***************************Section that makes buff1 and buff2 even-lengthed ****** */
        if( n > m )
        {
            ...
            //if n is even BUT m is not even
            else 
            {
...
/***************************************************************************** */
        ...
        {
            gradeschool = Gradeschool(a, b); free(gradeschool); 
        }
        ...
        long duration = (end_time.tv_sec - start_time.tv_sec) + (end_time.tv_nsec - start_time.tv_nsec);
        printf("Time taken by gradeschool: %lf micro seconds\n\n", (double)duration/1000);
        free(gradeschool); 
        ...
        duration = (end_time.tv_sec - start_time.tv_sec) + (end_time.tv_nsec - start_time.tv_nsec);
        printf("Time taken by karatsuba: %lf micro seconds\n\n", (double)duration/1000);
        free(karatsuba); 
